...
import numpy as np
from viola.embedding.model import EmbedModel
from viola.config.config import CONNECTION_MANAGER_PROXY, config


class Encoder(torch.nn.Module):
    def __init__(self, input_shape=256, num_feature=32, out_size=100):
        super().__init__()
        assert isinstance(input_shape, int), "need to be a integer."
        ...
        self.c0 = torch.nn.Conv2d(3, num_feature, kernel_size=5, stride=2)
        self.c1 = torch.nn.Conv2d(num_feature, num_feature*2, kernel_size=5, stride=2)
        ...
        shape_M = torch.randn([2,3,self.input_shape,self.input_shape])
        shape_M = self.c1(self.c0(shape_M)).shape
        
        self.M_shape = list(shape_M)[-2:]
        self.M_channels = num_feature*2

        in_feature = num_feature*8 * (self.M_shape[0]-3*2) * (self.M_shape[1]-3*2)
...
class DeepInfoMaxLoss(torch.nn.Module):
    ...
    def __init__(self, alpha=0.5, beta=1.0, gamma=0.1,encoder=None):
        super().__init__()

        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma
        self.M_channels = encoder.M_channels
        self.M_shape = encoder.M_shape
        self.out_size = encoder.out_size
        self.get_models()

    def get_models(self, interm_size_G=512, interm_channels_L=512, interm_size_P=(500,200) ):

        self.global_D = GlobalDiscriminator(M_channels=self.M_channels, M_shape=self.M_shape, E_size=self.out_size, interm_size=interm_size_G)
        self.local_D = LocalDiscriminator(M_channels=self.M_channels, E_size=self.out_size, interm_channels=interm_channels_L)
        self.prior_D = PriorDiscriminator(E_size=self.out_size, interm_size=interm_size_P)
    def forward(self, Y, M, M_fake):
        # see appendix 1A of https://arxiv.org/pdf/1808.06670.pdf

        Y_replicated = Y.unsqueeze(-1).unsqueeze(-1)
        Y_replicated = Y_replicated.expand(-1, -1, *self.M_shape)
        ...
        eps = 1e-9 #prevent Nan from log(0)
        ...
        mi_loss = local_loss + global_loss + prior_loss
        encoder_loss = local_loss + global_loss - prior_loss
        return encoder_loss,mi_loss


def get_models(alpha=0.5, beta=1.0, gamma=0.1, input_shape=256, num_feature=16, out_size=100):
    encoder = Encoder(input_shape= input_shape, num_feature=num_feature, out_size=out_size)

    mi_estimator = DeepInfoMaxLoss(alpha=0.5, beta=1.0, gamma=0.1,encoder=encoder)
